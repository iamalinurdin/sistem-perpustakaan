		...
		<div class="box-body">
			<form method="post" action="{{ route('admin.book.update', $book) }}" enctype="multipart/form-data">
				...
				<div class="form-group @error('title') has-error @enderror">
					<label>Judul</label>
					<input type="text" name="title" placeholder="masukan judul buku" class="form-control" 
				...
				<div class="form-group @error('short_description') has-error @enderror">
					...
					<textarea class="form-control" rows="3" name="short_description" placeholder="tuliskan deskripsi">{{ old('short_description') ?? $book->short_description }}</textarea>
				...
				<div class="form-group @error('quantity') has-error @enderror">
					<label>Jumlah</label>
					<input type="text" name="quantity" placeholder="masukan judul buku" class="form-control" value="{{ old('quantity') ?? $book->quantity }}">
					@error('quantity')
						<span class="help-block">{{ $message }}</span>
					@enderror
				</div>
